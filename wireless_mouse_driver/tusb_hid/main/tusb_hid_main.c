 ...
 */
const uint8_t* hid_string_descriptor[5] = {
    // array of pointer to string descriptors
    (uint8_t[]){0x09, 0x04},       // 0: is supported language is English (0x0409)
    "Kami",                     // 1: Manufacturer
...
static bool current_lmb_dir = MOUSE_BUTTON_UP;
static bool current_rmb_dir = MOUSE_BUTTON_UP;
...
static void mb_latch_task(void *arg)
{
    ...
    while (1) {
        bool saw_latch_event = lmb_latch_event || rmb_latch_event;
...
static bool mmb_dir = MOUSE_BUTTON_UP;
static bool smb4_dir = MOUSE_BUTTON_UP;
static bool smb5_dir = MOUSE_BUTTON_UP;
...
static bool swheel_a_state = SWHEEL_A_LOW;
static bool swheel_b_state = SWHEEL_B_LOW;

static bool swheel_dir = SCROLL_WHEEL_NONE;
...
{
    ESP_ERROR_CHECK(spi_bus_initialize(HSPI_HOST, &sensor_spi_bus_config, 1));
    ESP_ERROR_CHECK(spi_bus_add_device(HSPI_HOST, &sensor_spi_device_config, &sensor_spi_device));
...
static const sensor_register_programming_sequence[][2] = {
    {0x00, 0x00}, // Reset
...
// Function to start the Pixart PAW3395 sensor.
...
#define US_TO_TICKS(us) (us / (pdMS_TO_TICKS(1) / 1000))
